...
> **Worker 观测状态源**：https://mazhi.icu/zhuguang/status.json （以观测时间和来源为准）
...
gantt
    ...
    Demo 演示 (60s 模拟联动与红线阻断) :crit, p2, 03:00, 04:00
...
- **04:00 - 07:00（后 3 分钟）**：保留指挥中心页面，靶向回答评委提问，可随时调阅 6 大模拟场景时序与目标集群拓扑。
...
> • **高风险人机协同（HITL）**：维修预算超限或商品强制报损时，Policy 门禁自动拦截并挂起，等待独立 Human 身份审批，移动端接入待外部实现；
> • **只增审计设计（目标数据库待验收）**：在数据库底层剥夺 `audit_log` 的 UPDATE 和 DELETE 权限，限制运行角色修改历史审计，真实库权限与合规仍需验证。”
