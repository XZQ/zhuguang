...
| 如何证明不是概念稿 | 6 个确定性冷柜场景、112 个测试用例、6 个 P0 Skill、12 个 P0 MCP、3 个可选知识 MCP 和可复现 Evidence 均在仓库内 |
...
| 自动化测试 | 112 项发现：110 通过、2 个 PolarDB 条件集成测试因无外部实例跳过 | `uv run --group dev python -W error::ResourceWarning -m unittest discover -v` |
...
- 模型：`qwen3.5-plus` 仅声明给目标 AgentTeams Manager/Worker；本地确定性 Demo、112 项测试和 M4 评测不调用 LLM。
...
2026-09-12 完成[运行接口复核修复](docs/assessments/runtime-review-followup.md)，当前完整回归 112 项发现、110 通过、2 条件跳过。阶段输出恢复、失败事务回滚及重新处置闭环已纳入回归；远端 CI 和目标部署需另行按发布 SHA 验证。
...
- `qwen3.5-plus` 只用于目标 AgentTeams Manager/Worker 的任务拆解、结构化协作和工具编排；本地 `uv run dianxun evaluate` 不调用它，因此 6/6 和 112 项测试不是模型效果指标。
