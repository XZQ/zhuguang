...
| 如何证明不是概念稿 | 6 个确定性冷柜场景、105 个测试用例、6 个 P0 Skill、12 个 P0 MCP、3 个可选知识 MCP 和可复现 Evidence 均在仓库内 |
...
| 自动化测试 | 105 项发现：103 通过、2 个 PolarDB 条件集成测试因无外部实例跳过 | `uv run --group dev python -W error::ResourceWarning -m unittest discover -v` |
...
- 模型：`qwen3.5-plus` 仅声明给目标 AgentTeams Manager/Worker；本地确定性 Demo、105 项测试和 M4 评测不调用 LLM。
...
2026-09-07 的七项修复及兼容说明见 [修复记录](docs/assessments/seven-finding-repairs.md)。完整本地回归为 105 项发现、103 通过、2 条件跳过；新增 CI 矩阵和 Docker 门禁尚未在 GitHub 运行。门户和手册 Markdown/HTML 从模板重建，既有 PDF 为历史版本。
...
5 个 Worker 的下载地址均固定到已完成双平台复现的不可变 commit `ac0bc213dd0a4108deba4474761a575c39cc2a94`；契约测试禁止退回可漂移的 `main` URL。
...
- `qwen3.5-plus` 只用于目标 AgentTeams Manager/Worker 的任务拆解、结构化协作和工具编排；本地 `uv run dianxun evaluate` 不调用它，因此 6/6 和 105 项测试不是模型效果指标。
