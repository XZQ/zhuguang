...
- Manager/Worker YAML 声明 `qwenpaw + qwen3.5-plus`；本地确定性 Demo、112 项测试、M4 评测和消融对照不调用 LLM。
...
- Evidence 关键字段 45/45；
- 适用阶段 Trace 26/26；
- 全量发现 112 项自动化测试：110 项通过，2 项外部 PolarDB 条件测试跳过；
...
- 当前 P0 均为自定义可复用 Skill。复赛规则不要求指定云厂商 Skill；是否满足以目标 AgentTeams 中的发现、加载、调用、版本和 Trace 证据为准。
