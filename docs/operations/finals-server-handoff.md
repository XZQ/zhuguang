...
维护日期：2026-09-16。供队友拉取代码后执行；任务状态只维护在[统一待办](../待办.md)的 F01–F08。
本手册中的外部步骤是操作与验收要求，不是已经运行成功的报告；隔离 PostgreSQL 测试或 Worker 认证恢复不能代替真实 AT／Human、托管 PolarDB 和授权门店验收。

范围协议 v2 的停写、备份验证、显式迁移、Worker 参数与不可直接回退旧程序的边界见[范围协议升级与交接](scope-v2-upgrade.md)。不能把原有 `db-bootstrap` 或拉取代码当成已完成这次数据升级。
...
部署前按实际版本差异核对数据库迁移，不假定没有迁移；Worker ZIP 的 Python 实现来源仍须核对固定包。运行时新增了证据门禁：旧记录缺少适用凭证时不能按成功关闭；应补证并由 Auditor 再验，不回填虚构回执。回滚应先保持停售、暂停新任务，再切回已留存的镜像摘要；不要为了恢复旧的宽松验收而解除限制。

## 2. 身份、数据库与镜像

外置 Docker Worker 的 Controller SA Token 与下表 MCP 业务身份不同；安装时复制一次会过期。既有比赛拓扑的短期投影、定时器、挂载检查和自然心跳验收见[外置 Worker 凭据续期](../../agentteams/ops/README.md)。不要关闭鉴权或改用管理员 Token 绕过故障。
...
| 条件集成测试 | `DIANXUN_TEST_POSTGRES_DSN`、`DIANXUN_TEST_POSTGRES_READONLY_DSN`、`DIANXUN_TEST_POSTGRES_RUNTIME_DSN`、`DIANXUN_ALLOW_TEST_DATABASE_RESET=1` |
...
该测试类仍是 2 项条件测试。第一项增加了真实月首日／越维护窗口拒绝、SAVEPOINT 后继续事务、分区创建，以及知识审核后的 PostgreSQL vector 写入和检索。向量测试使用本地 hash embedding，仅证明数据库调用链，不证明语义模型效果。第一项内还执行四条 HTTP 协议回归：重启后独立关闭、身份/租约伪造拒绝、诊断响应丢失恢复、复核失败后重新遏制执行。须预建继承 `dianxun_runtime` 的独立 LOGIN；运行 DSN 的用户名不能与管理员或只读登录相同，且不能具有 superuser、BYPASSRLS、CREATEDB、CREATEROLE 权限。管理员仅准备合成夹具，HTTP 请求使用实际受限数据库连接；缺失运行 DSN 会失败，不静默跳过。第二项验证独立只读登录和总部／门店边界。**必须报告外层 2 passed、0 skipped，内层 4 passed、0 skipped**。这些合成 Worker/Human/实物回执不替代真实 AT、人工审批或托管 PolarDB 验收。
...
部署前同时检查 `SHOW cron.timezone`、`SHOW timezone` 以及作业登录的数据库/角色时区覆盖。`0 23 * * *` 按调度器时区触发，而 SQL 的 `CURRENT_DATE` 取实际作业会话时区，不能仅凭宿主时区推断。国内单时区演示须由 DBA 明确配置并验证两者为 `Asia/Shanghai`；多时区租户的逐租户业务日期与调度属于另行设计，现有全局作业不证明已支持。

2026-09-16 已对 `e2b6b5a` 的原 cron 命令完成独立 PG 秒级加速实测：两租户入队 2 条，后续运行 `INSERT 0 0`；自动创建缺失的次月分区；故意除零的 runid 7、9 在健康视图显示 failed，恢复原命令后 runid 11 succeeded。结束时停用作业并停止专用测试容器，保留运行历史。证据路径与完整进度见[统一待办](../待办.md)。此结果不代替自然夜间/月度调度、外部告警投递、托管 PolarDB 或真实复盘执行。
