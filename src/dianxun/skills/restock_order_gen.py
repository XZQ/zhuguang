...
九要素（详见 skills/planned/restock-order-gen.md）：
  ...
  输出    RestockOrder{items[{sku, suggest_qty, current_stock, daily_sales,
          days_to_empty, priority}], total_amount}
...
def restock_order_gen(
    ...
    with trace.span(
        "restock-order-gen", "skill", tid, input={"store_id": store_id, "urgency": urgency}
    ) as sp:
        ...
        price_map = (
            {r["sku_id"]: r["system_price"] for r in price_res["rows"]}
            if not price_res["degraded"]
            else {}
        )
        ...
        order = {
            ...
            "confidence": 0.82,
            "comments": "草稿单,需店长/采购审批后提交"
            + (";金额>5000强制审批" if need_approval else ""),
        }
