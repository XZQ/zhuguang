...
def _console(message: str) -> None:
    """Write human-facing progress without failing on a legacy Windows code page."""
    try:
        print(message)
            ...
            with trace.span(
                ...
                for anom in ctx.anomalies:
                    ...
                    if not ok:
                        ...
                        second_ok = self.auditor.verify(anom, ctx)
                        if not second_ok:
                            ctx.transition("failed", "Auditor", "二次验证仍失败,停止错误闭环")
                            sp.output = {"result": "failed", "reason": "second verification failed"}
                            return {
                                "task_id": task_id,
                                "result": "failed",
                                "trace_id": tid,
                ...
                _console(f"✓ 任务 {task_id} 闭环 | 共处置 {len(ctx.anomalies)} 个异常")
                return report
        except Exception as e:  # noqa: BLE001
            _console(f"✗ 任务 {task_id} 异常: {type(e).__name__}: {e}")
            if ctx.state not in {"closed", "failed"}:
                ctx.transition("failed", "Orchestrator", f"异常终止:{type(e).__name__}")
            raise
...
class SentryAgent:
    ...
    def detect(self, ctx: TaskContext) -> None:
        _console(f"  🔍 [Sentry 巡检] 检测范围:{ctx.scope}")
        ...
        if result.get("degraded"):
            _console(f"     ⚠ 数据源降级,进入降噪模式:{result.get('error')}")
        # 高危优先(已按严重度排序)
        high = [a for a in ctx.anomalies if a["severity"] in ("高", "严重")]
        _console(f"     检出 {len(ctx.anomalies)} 个异常(高危 {len(high)} 个)")
...
class DiagnoserAgent:
    ...
    def diagnose(self, anomaly: dict, ctx: TaskContext, retry: bool = False) -> None:
        ...
        if metric in ("temp", "stockout_rate", "price_mismatch_rate"):
            ...
            _console(f"     跨店对标:{bench.get('conclusion')} (zscore={bench.get('zscore')})")
        rc = skills.rootcause_drilldown(anomaly, bench, trace_id=ctx.trace_id)
        _console(f"     根因:{rc['hypothesis']} (置信度 {rc['confidence']})")
...
class ExecutorAgent:
    ...
    def handle(self, anomaly: dict, ctx: TaskContext) -> None:
        ...
        _console(f"  🔧 [Executor 处置] {atype}@{sid}")
        action: dict
        if atype == "冷柜超温":
    ...
    def _handle_coldchain(self, anomaly: dict, ctx: TaskContext) -> dict:
        # 维修工单(>2000 走审批)
        budget = 2500.0
        apr = mcp.create_approval(
            f"{anomaly['store_id']}冷柜维修",
            "workorder",
            {"budget": budget, "fault": "压缩机故障"},
            ["店长"],
        )
        apr_id = (
        ...
        return {
            ...
            "approval_id": apr_id,
            "workorder": wo,
            "executed": not wo.get("blocked", False),
        }
    ...
    def _handle_stockout(self, anomaly: dict, ctx: TaskContext) -> dict:
        order = skills.restock_order_gen(
            ...
            urgency="紧急",
            sku_list=(
                [anomaly["evidence"].get("sku_id")] if anomaly["evidence"].get("sku_id") else None
            ),
            trace_id=ctx.trace_id,
        )
        ...
        if order.get("need_approval"):
            apr = mcp.create_approval(
                f"{anomaly['store_id']}补货单",
                "restock",
                {"total": order.get("total_amount")},
                ["采购"],
            )
            executed = (
                apr["rows"].get("status") == "approved" if isinstance(apr["rows"], dict) else True
            )
        return {
            "type": "restock",
    ...
    def _handle_price(self, anomaly: dict, ctx: TaskContext) -> dict:
        ...
        items = [
            {
                "sku_id": anomaly["evidence"]["sku_id"],
                "new_price": anomaly["evidence"]["system_price"],
            }
        ]
        apr = mcp.create_approval(
            f"{anomaly['store_id']}价签纠正", "price_change", {"items": items}, ["店长"]
        )
        apr_id = (
            apr["rows"]["approval_id"]
            if isinstance(apr["rows"], dict)
...
class AuditorAgent:
    ...
    def verify(self, anomaly: dict, ctx: TaskContext) -> bool:
        _console(f"  ✅ [Auditor 验证] {anomaly['type']}@{anomaly['store_id']}")
        atype = anomaly["type"]
        # demo:规则核验(生产换 IoT 复测/POS 复查)
        if atype == "冷柜超温":
            # 模拟:工单完成后温度回落
            ok = any(
                a.get("type") == "work_order" and a.get("executed")
            ...
            ok = any(
                a.get("type") == "restock" and a.get("executed")
                for a in ctx.actions
            ...
            ok = any(
                a.get("type") == "price_change" and a.get("executed")
                for a in ctx.actions
    ...
    def review(self, ctx: TaskContext) -> dict:
        _console("  📝 [Auditor 复盘] 沉淀知识 + Skill 建议")
