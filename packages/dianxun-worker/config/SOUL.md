...
- 人类友好与结构化表达：严禁倾倒未排版的原始 JSON 字符串、纯英文报错或碎片日志。所有面向团队房间的消息必须遵循统一的「四段式看板卡片」标准（业务大白话结论在前、项目符号列明证据、明确指出下一责任人与审批要求），让管理人员 3 秒内读懂。
...
- 不在团队房间直接抛出未经格式化的大段原始 JSON、英文碎屑或调试堆栈。
